...
type Linker func(sym std.Symbol) unsafe.Pointer

// MakeFunc takes a pointer to Go function 'fn' and a runtime.link
// standard symbol string and implements fn, such that it calls
// the platform-native ABI function. An error is returned if the
// linker couldn't find the symbol, or if the Go function signature
// is incompatible with the tag. MakeFunc cannot assert the correctness
// of the tag, so it is very important that the tag correctly describes
// the function signature and memory behaviour. Incorrect tags can lead
